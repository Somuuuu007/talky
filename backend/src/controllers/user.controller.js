...
export async function sendFriendRequest(req, res) {
    try {
        ...
        if(existingRequest) {
            return res.status(400).json({message: "A friend request has already been sent to this user"});
        }
...
export async function getFriendRequests(req, res) {
    try{
        ...
        console.log("Error getting friend requests", error.message);
        res.status(500).json({message: "Internal server error"});
    }
}
...
export async function getOutgoingFriendRequests(req, res) {
    try {
        ...
        console.log("Error getting outgoing friend requests", error.message);
        res.status(500).json({message: "Internal server error"});
    }
}